...
const AnimatedButton = ({ label, labelColor, bgColor, url, fullWidthOnMobile = false }) => {
  const [isActive, setIsActive] = useState(false);
  const [isMobile, setIsMobile] = useState(false);
  ...
  const buttonStyle = {
    color: labelColor,
    backgroundColor: bgColor,
    paddingRight: isActive ? "20px" : "0px",
  ...
  return (
    <div className={fullWidthOnMobile && isMobile ? "w-full" : ""}>
      <Link to={url} className={fullWidthOnMobile && isMobile ? "w-full block" : ""}>
        <motion.button
          className={`px-5 py-2 rounded-lg transition-all duration-300 ease-in-out flex items-center justify-center text-[16px] font-semibold ${fullWidthOnMobile && isMobile ? "w-full" : ""}`}
          style={buttonStyle}
          onMouseEnter={isMobile ? null : () => setIsActive(true)}
          onMouseLeave={isMobile ? null : () => setIsActive(false)}
          onTouchStart={handleTouchStart}
